...
define(function( require )
{
	...
	EffectManager.has = function has( effect, AID, effectID )
	{
		function hasEffect( effectList ) {
			return effectList.find( ( effectEntry ) => {
				return ( !!AID && effectEntry._Params.Init.ownerAID === AID && !!effectID && effectEntry._Params.Inst.effectID === effectID );
			} );
		};

		if( !!effect ) {
			return hasEffect( _list[effect.name] );
		}
		else {
			return Object.keys( _list ).find( ( effectName ) => {
				return hasEffect( _list[effectName] );
			} );
		}
